...
const NotificationAlert = props => {
    ...
    const acceptRoutine =  () => {
        console.log('Accept Routine')
    }

    const cancelRoutine =  () => {
        console.log('Cancel Routine')
    }
                        ...
                        <ul className="p-0">
                            {
                                routineNotificationList.map((item, i) => {
                                    return <Alert style={{backgroundColor: '#EBEBEB', marginTop: '10px'}} key={i}>
                                        <i className="fas fa-circle" style={{backgroundColor: '#EBEBEB', padding: '10px'}}></i>You have to take 1 piece {item.itemName} tablet at 8:00 PM.
                                        <div className="btn-group float-md-right" role="group" aria-label="Basic example">
                                            <div className="row ml-2 ml-sm-0">
                                                <div className="col-12" style={{borderRadius: '1em', backgroundColor: '#0C0C52'}}>
                                                    <button className="btn-block btn text-white" onClick={function() {acceptRoutine()}} disabled = {(disable)? "disabled" : ""}>
                                                        Confirm
                                                    </button>
                                                </div>
                                            </div>
                                            <div className="row ml-4 ml-sm-4 mr-sm-1 ml-md-4 mr-md-1">
                                                <div className="col-12 bg-danger" style={{borderRadius: '1em'}}>
                                                    <button className="btn-block btn text-white" onClick={function() {cancelRoutine()}} disabled = {(disable)? "disabled" : ""}>
                                                    Delete
                                                    </button>
                                                </div>
                                            </div>
